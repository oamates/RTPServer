...
//	LRESULT NotifyHandler(int /*idCtrl*/, LPNMHDR /*pnmh*/, BOOL& /*bHandled*/)
	...
	LRESULT OnInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
	{
		...
		TCHAR ach[256];
		wsprintf(ach, _T("Duration: %d seconds"), m_Duration);
...
LRESULT CMainDlg::OnInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
{
    ...
    {
        MessageBox(_T("Please register GMFBridge.DLL with regsvr32"));
...
LRESULT CMainDlg::OnBnClickedAdd(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
	...
	if (GetOpenFileName(&ofn)) 
	{
		...
		if (FAILED(hr))
		{
			MessageBox(_T("Cannot add clip: format not support"), _T("Incompatible Clip"));
			return 0;
		}

		wstring sDesc = pClip->Name();
		...
		if (dlg.DoModal() == IDOK)
		{
			...
			TCHAR ach[128];
			wsprintf(ach, _T(" [%d..%d]"), long(tStart/UNITS), long(tStop/UNITS));
			sDesc += ach;
		}
...
LRESULT CMainDlg::OnBnClickedLimits(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
    ...
    if (pClip != NULL)
    {
		...
		if (dlg.DoModal() == IDOK)
		{
			...
			// append range to clip description text
            wostringstream strm;
            strm << sDesc << _T(" [") << long(tStart/UNITS) << _T("..") << long(tStop/UNITS) << _T("]");
            sDesc = strm.str();
