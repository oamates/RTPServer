...
: m_bLoop(false),
  ...
  m_pPlayNext(NULL)
{
    HRESULT hr;// = CComObject<BridgeController>::CreateInstance(&m_pController);
    m_pController = (BridgeController*)BridgeController::CreateInstance(NULL, &hr);
...
HRESULT 
ClipPlayer::AddClip(const TCHAR* path, ClipEntry** ppClip)
{
...
HRESULT 
ClipEntry::Create(BridgeController* pController, const TCHAR* path)
{
    m_bPrimed = false;
...
HRESULT AddToROT(IGraphBuilder* pGraphBuilder)
{
	...
	WCHAR wsz[256];
	swprintf_s(wsz, _countof(wsz), L"FilterGraph pid %08x", GetCurrentProcessId());
	if (SUCCEEDED(hr = GetRunningObjectTable(0, &pROT))
		&& SUCCEEDED(hr = CreateItemMoniker(L"!", wsz, &pMoniker)))
	{
		hr = pROT->Register(ROTFLAGS_REGISTRATIONKEEPSALIVE, pGraphBuilder, pMoniker, &m_dwRegister);
	}
