...
PreviewController::SelectDevice(const TCHAR* pVideo, const TCHAR* pAudio, HWND hwnd, RECT* prc)
{
    ...
    HRESULT hr;
    m_pBridge = (BridgeController*)BridgeController::CreateInstance(NULL, &hr);
	if (m_pBridge == NULL)
	{
		return E_FAIL;
	}
...
HRESULT PreviewController::AddToROT(IGraphBuilder* pGraphBuilder)
{
	HRESULT hr;
	...
	WCHAR wsz[256];
	swprintf_s(wsz, _countof(wsz), L"FilterGraph %08p pid %08x", this, GetCurrentProcessId());
...
HRESULT PreviewController::RemoveFromROT()
{
	HRESULT hr;
